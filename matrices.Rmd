...
```{r, results="hide"}
read.csv("intro-r/pvc.csv", row.names=1)
```
...
>
> The filename "intro-r/pvc.csv" means from the current working directory, in the sub-directory "intro-r", the file "pvc.csv".
>
...
```{r, results="hide"}
dat <- read.csv("intro-r/pvc.csv", row.names=1)
```
