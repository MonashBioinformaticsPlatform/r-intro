...
>
> The filename means from the current working directory, in the sub-directory "data", in the sub-directory "intro-r", the file "pvc.csv".
>
...
`read.csv` has two arguments: the name of the file we want to read, and which column contains the row names.
The filename needs to be a character string, so we put it in quotes. Assigning the second argument, `row.names`, to be `1` indicates that the data file has row names, and which column number they are stored in.
