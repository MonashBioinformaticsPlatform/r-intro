...
Data frames are very central to R, and mastering R is very much about thinking in data frames. However anything statistical will often involve using matrices. For example when we work with RNA-Seq data we use matrices of read counts. So it will be worth our time to learn to use matrices as well.
...
>
> We used `class` earlier to see what type various values were. Here this tells us it is an `"htest"`, but this is actually just the "public face" of the value. Sometimes we need to Scooby Doo a value and see how R is thinking of it under the hood. `mode` reveals how R is thinking of a value internally. If we uncover that the `mode` of an object is `"list"`, we then know we can use `$` or `[[ ]]` to access its elements.
>
> ```{r}
> class(result)
> mode(result)
...
>
> Advanced: How would you calculate the average plus two standard deviations, or the average minus two standard deviations?
>
