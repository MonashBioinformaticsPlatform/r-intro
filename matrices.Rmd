...
```{r, results="hide"}
read.csv(file="data/intro-r/pvc.csv", row.names=1)
```

> ### Tip {.callout}
>
> The location of the file is given relative to your "working directory". You can see the location of your working directory in the title of the console pane in RStudio. It is most likely "~", indicating your personal home directory.
>
...
>
> If you are working on your own machine rather than our training server, and downloaded and unarchived the intro-r.zip file, the files may be in a different location.
>
...
If we want to get a single value from the data frame, we can provide a row and column index in square brackets:
...
>
> 3.  Use a slice of `phrase` to create a new character vector that spells the phrase "I know I don't", i.e. `c("I", "know", "I", "don't")`.
>
...
`plot` can take many different parameters to modify the appearance of the output. Here is a plot with some different parameters:
...
>
> Extension: Create similar plots for operator. Comment on which dimension is the major source of variation in this data.
>
