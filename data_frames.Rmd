...
> ### Tip {.callout}
>
> A data frame can also be created de novo from vectors, with the `data.frame` function. For example
>
> ```{r}
...
There is also an `is.na` function, allowing us to perform this removal manually using logical indexing.

```{r}
not_na <- !is.na(diabetes$glyhb)
mean( diabetes$glyhb[ not_na ] )
```
...
When we gave R two factors to plot it produces a "mosaic plot" that helps us see if there is any relationship between the two factors.
...
2. The what we want *not* to be collapsed in the output.
...
The result is a vector, with names from the classifying factor. These means seem of a continuous measurement seem to be bearing out our earlier observation using a discrete form of the measurement that this data show some link between body frame and diabetes prevalence.
...
```{r}
cholesterol <- read.csv("data/intro-r/chol.csv")
head(cholesterol)
```
...
Oh. The two data frames don't have exactly the same set of subjects. We should also have checked that they were even in the same order before blithely combining them. R has shown an error this time, but there are many ways to mess up like this that would not show an error. How embarassing.
