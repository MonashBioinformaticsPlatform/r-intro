...
Seasoned programmers may notice with some alarm that `aes` is a magical function, since its bare parameters refer to columns of the data frame as though they were variables. To those programmers we comment briefly and inscrutibly that R uses lazy evaluation of function parameters to allow any function to potentially behave like a macro.
...
We can see some trend for glyhb to increase with age, and we tend to see medium and large framed people at higher levels of glyhb. Lets try to show this trend.

```{r}
ggplot(na.omit(diabetes), aes(y=weight, x=height, color=frame)) +
...
> ```
>
> Plot life expectancy vs GDP per capita, coloring points by continent.
>
