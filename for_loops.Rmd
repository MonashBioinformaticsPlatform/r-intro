...
First let's get a character vector of the filenames we want to load. I'm using a couple of useful functions we haven't seen before, `seq` and `paste0`. As usual, you can look these up in the help system with `?seq` and `?paste0`.
...
```{r}
filenames <- paste0("intro-r/gapminder-", years, ".csv")
filenames
# We could also have just written
# filenames <- c("intro-r/gapminder-1952.csv", <etc> )
```

It would be nice if R would load all of these at once. Many R functions do behave like this. Alas in this case `read.csv` gives an obscure error message.

```{r}
gap <- read.csv(filenames)
```

We will loop over all of the filenames, and build up a data frame. We start with `NULL`, which is a special value in R meaning nothing at all. We add to this with `rbind`, which concatenates the rows of data frames.
